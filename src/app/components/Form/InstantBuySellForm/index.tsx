...
class InstantBuySellForm extends React.Component<any, any>  {
  ...
  checkLogin = () : boolean => {
    if (!this.props.account.content.id) {
    ...
    return (
        <div className='instantbuysell'>
          <div className='grid-4'>
            <button type='button' className='buy' disabled={pristine || submitting} onClick={ this.buyOrder}><FormattedMessage id='spot_usd_landing_button_buy' /></button>
          </div>
          <div className='grid-4'>
            <div className={`input-group ${this.state.showError ? 'error' : ''}`}>
              <span className='input-group-addon'><FormattedMessage id='spot_usd_landing_input_quantity' /></span>
            ...
            <span className='price'> ≈ <FormattedNumber value={this.props.ticker.last * this.state.quantity} minimumFractionDigits={2} maximumFractionDigits={2} />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;USD</span>
          </div>
          <div className='grid-4'>
            <button type='button' className='sell' disabled={pristine || submitting} onClick={ this.sellOrder}><FormattedMessage id='spot_usd_landing_button_sell' /></button>
          </div>
        </div>
    );
  }
